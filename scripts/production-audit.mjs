...
import {inspectPrimaryVisual,baseline} from './round7-audit.mjs';
...
const articleImages=JSON.parse(readFileSync('src/data/article-images.json','utf8'));
...
for(const page of expected.urls){
  ...
  if(page.article){const id=articleSource.find(a=>a.data.canonical===page.url)?.id;if(id&&articleImages[id])errors.push(...inspectPrimaryVisual(html,id,articleImages[id]).map(e=>page.url+': '+e));}
  for(const match of html.matchAll(/<img\b[^>]*src="([^"]+)"/g))assetUrls.add(new URL(match[1],page.url).href);
...
for(const url of assetUrls){
  ...
  const valid=response.status===200&&(isPng?type?.includes('image/png')&&body[0]===137&&body[1]===80:isSvg?type?.includes('image/svg+xml')&&new TextDecoder().decode(body).includes('<svg'):url.endsWith('.css')?type?.includes('text/css'):type?.startsWith('image/'));
...
const legacy=await fetch(base+'/produk-laser-cutting',{redirect:'manual'});const legacyLocation=legacy.headers.get('location');if(legacy.status!==301||!legacyLocation||new URL(legacyLocation,base).href!==base+'/portfolio/')errors.push('Legacy URL must return 301 to /portfolio/');
...
const report={runAt,checkedUrls:pages.length,newArticleHttp200:pages.filter(p=>p.status===200&&articleSource.some(a=>a.data.canonical===p.url&&!baseline.includes(a.id))).length,articleConversionCoverage:articleCoverage,errors,externalActions:external,sitemap:{status:sitemapResponse.status,urls:sitemapUrls.length},robots:{status:robotsResponse.status},legacyRedirect:{status:legacy.status,location:legacyLocation},missingStatus:missing.status,httpRedirect:{status:http.status,location:http.headers.get('location')},www:{status:www.status,location:www.headers.get('location')},assets,pages};
