...
const transport=arg('transport','default');
if(!['default','http1'].includes(transport))throw new Error('Use --transport=default or --transport=http1');
...
const chromeFlags=['--headless=new','--disable-gpu',...(transport==='http1'?['--disable-http2','--disable-quic']:[])];
const chrome=await chromeLauncher.launch({chromePath:process.env.CHROME_PATH||undefined,chromeFlags,userDataDir:profile,handleSIGINT:false});
const runs=[];
try{
  for(const device of devices)for(const path of selected){
    ...
    const {lhr}=result;
    const response=(result.artifacts?.DevtoolsLogError||result.artifacts?.DevtoolsLog)?.filter(e=>e.method==='Network.responseReceived').map(e=>e.params.response).find(r=>r.url===base+path);
    const headers=Object.fromEntries(Object.entries(response?.headers||{}).map(([key,value])=>[key.toLowerCase(),value]));
    const documentResponse=response?{status:response.status,protocol:response.protocol,server:headers.server,requestId:headers['x-hcdn-request-id']}:null;
    const lcp=lhr.audits['largest-contentful-paint-element'];
    const run={path,device,time:lhr.fetchTime,version:lhr.lighthouseVersion,finalUrl:lhr.finalDisplayedUrl||lhr.finalUrl,scores:Object.fromEntries(Object.entries(lhr.categories).map(([k,v])=>[k,typeof v.score==='number'?Math.round(v.score*100):null])),metrics:Object.fromEntries(['first-contentful-paint','largest-contentful-paint','total-blocking-time','cumulative-layout-shift','speed-index'].map(k=>[k,lhr.audits[k]?.numericValue])),lcpElement:lcp?.details||null,documentResponse,runtimeError:lhr.runtimeError||null,warnings:lhr.runWarnings,failedAudits:Object.values(lhr.audits).filter(a=>a.score!==null&&a.score<1&&!['informative','manual','notApplicable'].includes(a.scoreDisplayMode)).map(a=>({id:a.id,score:a.score,title:a.title}))};
    runs.push(run);
    writeFileSync(resolve(cache,`${production?'production':'local'}-${device}-${path.replace(/[^a-z0-9]/g,'_')||'home'}.json`),result.report);
    writeFileSync(out,JSON.stringify({base,transport,chromeFlags,environment:production?'Real production URLs, headless Chrome, Lighthouse simulated throttling; not field data.':'Local static production build, headless Chrome, Lighthouse simulated throttling.',runs},null,2)+'\n');
    console.log(JSON.stringify({path,device,scores:run.scores,lcpMs:run.metrics['largest-contentful-paint'],cls:run.metrics['cumulative-layout-shift'],documentResponse,runtimeError:run.runtimeError?.message}));
  }
